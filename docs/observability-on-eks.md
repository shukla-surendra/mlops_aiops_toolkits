...
- 2026-08-04: Initial write-up — three-pillar primer, EKS integration-flow
  diagram, responsibility matrix, Loki-vs-ELK/EFK comparison, tracing
  layer, alerting layer, managed alternatives, and recommended stacks.
- 2026-08-04: Fixed a real gap flagged in review — the original diagram
  showed Prometheus/Loki/Elasticsearch/Tempo as terminal stores with no
  persistence layer underneath. Added a "Storage" subgraph to the diagram
  and a new section explaining each store's actual durability mechanism on
  EKS: Prometheus's local EBS-backed PVC (short retention) plus optional
  Thanos/Mimir/Cortex remote_write to S3 for real long-term retention;
  Loki's native S3/GCS/Azure Blob chunk store (its core design, not an
  afterthought); Elasticsearch's EBS-backed replica shards with optional
  ILM+snapshot-to-S3; Tempo's S3-backed trace blocks. Also split the
  diagram's combined "Loki or Elasticsearch" node into two separate nodes
  since their storage models differ enough to matter.
- 2026-08-04: Added a dedicated CloudWatch comparison section — how
  Container Insights/Logs/Metrics/Alarms/X-Ray show up on EKS, and a
  comparison table across setup effort, storage durability, query
  language, retention, cost model, and portability against Prometheus+
  Grafana, Loki, and Elasticsearch/ELK-EFK. Framed around the fact that
  CloudWatch sidesteps the storage-durability question from the previous
  entry entirely — AWS owns it — which is its core trade-off (convenience
  vs. portability/cost-at-scale/query power).
- 2026-08-04: Added a second Mermaid diagram for the AWS-native path —
  pods -> CloudWatch Agent/Fluent Bit + X-Ray daemon/ADOT -> CloudWatch
  Logs/Metrics + X-Ray -> fully-managed storage (one box, in contrast to
  the self-hosted diagram's whole Storage subgraph) -> Logs
  Insights/Dashboards/Service Map -> Alarms -> SNS, plus optional
  Grafana/AMG querying CloudWatch as a data source.
